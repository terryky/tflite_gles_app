...
void main() {
  ...
  for (int l = 0; l < src_depth; ++l) {
    vec4 input0 = Vec4FromHalf(input_data_0.data[gid.x + input_data_0_w * (gid.y + input_data_0_h * (l))]);
  ...
  output_data_0.data[gid.x + output_data_0_w * ( gid.y + output_data_0_h * ( gid.z))] = Vec4ToHalf(value_0);
}
