...
void main() {
  ...
  for (int l = 0; l < src_depth; ++l) {
    vec4 input0 = input_data_0.data[gid.x * 2 + 0 + input_data_0_w * (gid.y + input_data_0_h * (l))];
    vec4 input1 = input_data_0.data[gid.x * 2 + 1 + input_data_0_w * (gid.y + input_data_0_h * (l))];
  ...
  output_data_0.data[gid.x * 2 + 0 + output_data_0_w * (gid.y + output_data_0_h * (gid.z))] = result0;

  result1 = clamp(result1, vec4(0.0), vec4(clip));
  output_data_0.data[gid.x * 2 + 1 + output_data_0_w * (gid.y + output_data_0_h * (gid.z))] = result1;
}
