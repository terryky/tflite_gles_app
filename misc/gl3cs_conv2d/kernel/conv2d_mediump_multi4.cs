...
void main() {
  ...
  for (int l = 0; l < src_depth; ++l) {
    vec4 input0 = Vec4FromHalf(input_data_0.data[gid.x * 4 + 0 + input_data_0_w * (gid.y + input_data_0_h * (l))]);
    vec4 input1 = Vec4FromHalf(input_data_0.data[gid.x * 4 + 1 + input_data_0_w * (gid.y + input_data_0_h * (l))]);
    vec4 input2 = Vec4FromHalf(input_data_0.data[gid.x * 4 + 2 + input_data_0_w * (gid.y + input_data_0_h * (l))]);
    vec4 input3 = Vec4FromHalf(input_data_0.data[gid.x * 4 + 3 + input_data_0_w * (gid.y + input_data_0_h * (l))]);
  ...
  output_data_0.data[gid.x * 4 + 0 + output_data_0_w * (gid.y + output_data_0_h * (gid.z))] = Vec4ToHalf(result0);

  result1 = clamp(result1, vec4(0.0), vec4(clip));
  output_data_0.data[gid.x * 4 + 1 + output_data_0_w * (gid.y + output_data_0_h * (gid.z))] = Vec4ToHalf(result1);

  result2 = clamp(result2, vec4(0.0), vec4(clip));
  output_data_0.data[gid.x * 4 + 2 + output_data_0_w * (gid.y + output_data_0_h * (gid.z))] = Vec4ToHalf(result2);

  result3 = clamp(result3, vec4(0.0), vec4(clip));
  output_data_0.data[gid.x * 4 + 3 + output_data_0_w * (gid.y + output_data_0_h * (gid.z))] = Vec4ToHalf(result3);
}
